module.exports = {
  ...
  oscInFilter: function(data) {
    ...
    if (address === '/key_pressure') {
      ...
      /
      // TODO: need to make all of these variables consistent in my excel file. how to determine the toggle state of a button as an input as well.
      for (let i = 0; i < 3; i++) {
        var art + i + name = tracks[x].Art + i + _Name
        var art + i + mode = parseInt(tracks[x].Art + i + _Mode)
        var art + i + type = tracks[x].Art + i + _Type
        var art + i + code = parseInt(tracks[x].Art + i + _Code)
        var art + i +
          default = parseInt(tracks[x].Art + i + _Default)
        var art + i + on = parseInt(tracks[x].Art + i + _On)
        //integer first two art buttons are toggles
        var art + i + off = parseInt(tracks[x].Art + i + _Off)
      }
      for (let i = 3; i < 19; i++) {
        var art + i + name = tracks[x].Art + i + _Name
        var art + i + mode = parseInt(tracks[x].Art + i + _Mode)
        ...
        var art + i + on = parseInt(tracks[x].Art + i + _On)
        //string
        var art + i + off = String(tracks[x].Art + i + _Off)
      }
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (art + i + name === "") {
          ...
          sendOsc({
            ...
            host: 'midi',
            //OSC3 means OSC will also receive this command
            port: 'OSC3'
          })
        }
      }
        ...
        if (art + i + name === "") {
          ...
          if (art + i +
            ...
            sendOsc({
              ...
              host: 'midi',
              //OSC4 means only Cubase will receive this command
              port: 'OSC4'
            })
          } else {
            receive('/art' + i + 'mode', 0.15)
      ...
      //rename this text ID
      receive('/text_1', tn)
      //rename these fader IDs or figure out how to do the code for alphabet??
      for (let i = 0; i < 9; i++) {
        receive('/CC' + String.fromCharCode(65 + i) + '_increment_value', String.fromCharCode(97 + i) + 1)
